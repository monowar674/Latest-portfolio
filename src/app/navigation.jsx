...
export const Navigation = () => {
  ...
  return (
    <>
      {/* <div className="w-full h-18 px-20 py-2 bg-gray-100 flex justify-between items-center"> 
        <div>
         <Image 
         src={logo}
         className="w-20 h-20 bg-none"
         />
        </div>
        <div>
          <ul>
      ...
      <div className="absolute z-10 top-0 pointer-events-none left-0 w-full">
        <nav className="relative bg-transparent top-0 left-0">
          <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
            <div className="relative flex h-18 items-center justify-between">
              <div className="absolute pointer-events-auto inset-y-0 left-0 flex items-center sm:hidden">
                ...
                <button onClick={() => setOpen(open === 'hidden' ? 'block' : 'hidden')} type="button" command="--toggle" commandfor="mobile-menu" className="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-white/5 cursor-pointer hover:text-white focus:outline-2 focus:-outline-offset-1 focus:outline-indigo-500">
              ...
              <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                ...
                <div className="hidden sm:ml-6 sm:block">
                  <div className="flex pointer-events-auto space-x-4 items-center">
                    {/* <Link onClick={()=>{
                      setActive(active === 'activate' ? 'nonactive' : 'activate')
                    }} href="/" aria-current="page" className="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white">Home</Link> */}
                    <Link onClick={()=>{
                      setActive(active === 'activate' ? 'nonactive' : 'activate')
                    }} href="/" className=" menue rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Home</Link>
                    <Link onClick={()=>{
                      setActive(active === 'activate' ? 'nonactive' : 'activate')
                    }} href="/about" className=" menue rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">About</Link>
                    <Link onClick={()=>{
                      setActive(active === 'activate' ? 'nonactive' : 'activate')
                    }} href="/project" className=" menue rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Projects</Link>
                    <Link onClick={()=>{
                      setActive(active === 'activate' ? 'nonactive' : 'activate')
                    }} href="/blog" className={`${active} menue  rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white`}>Blog</Link>
                  </div>
                </div>
              </div>
          ...
          <el-disclosure id="mobile-menu" className={`${open} pointer-events-auto sm:hidden transition duration-200 ease-in-out`}>
            <div className="space-y-1 px-2 pt-2 pb-3">
              <Link  onClick={() => setOpen(open === 'hidden' ? 'block' : 'hidden')} href="/" aria-current="page" className="block rounded-md bg-gray-900 px-3 py-2 text-base font-medium text-white">Home</Link>
              <Link  onClick={() => setOpen(open === 'hidden' ? 'block' : 'hidden')} href="/about" className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white">About</Link>
              <Link  onClick={() => setOpen(open === 'hidden' ? 'block' : 'hidden')} href="/project" className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white">Projects</Link>
              <Link  onClick={() => setOpen(open === 'hidden' ? 'block' : 'hidden')} href="/blog" className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white">Blog</Link>
            </div>
          </el-disclosure>
        </nav>
      </div>
